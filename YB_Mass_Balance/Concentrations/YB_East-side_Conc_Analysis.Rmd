...
```{r filter major flood events and eastern stations}
major_flood_east <- 
  # Create a nested tibble
  tibble(
    name = c("sourceID", "parameters", "hg_stack", "hg_fill"),
    df = list(conc_spcond_filt, conc_param, conc_hg, conc_hg)
  ) %>% 
  mutate(
    df = map(
      df,
...
```{r source id plots 1, fig.height = 9, fig.width = 10}
...
```{r source id plots 3, fig.height = 9, fig.width = 10}
source_id_plots %>% 
