...
# Round load values before calculating the percentages
loads_inlet_2017_r <- loads_inlet_2017 %>% 
...
loads_inlet_rank <- loads_inlet_2017_r %>% 
...
loads_inlet_part_avg <- loads_inlet_hg_frac %>% 
  # Round each load value before calculating percentages
  ...
  mutate(per_part = particulate/total) %>% 
...
loads_inlet_total_part_avg <- loads_inlet_hg_frac %>% 
  ...
  summarize(
    sign_digits = min(digits),
    load_sum = signif(sum(Load), sign_digits)
  ) %>% 
  ungroup() %>%
  # Calculate percentage of particulate fraction
  pivot_wider(
