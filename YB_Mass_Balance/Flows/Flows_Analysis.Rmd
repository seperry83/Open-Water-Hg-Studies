...
flows_inlet_all <- daily_flow_data_all %>% 
  filter(
    Year != 2014,
    LocType == "Inlet"
  )
...
# Sum the CCSB Flows
ccsb_flow <- filter(flows_inlet_all, str_detect(StationName, "^CCSB"))
...
flows_inlet_all <- flows_inlet_all %>% 
  anti_join(ccsb_flow) %>% 
  bind_rows(ccsb_flow_sum)
...
# Calcuate the averages of the percentages of Fremont Weir when it was spilling for the
  # 2016 and 2017 floods
...
# Calculate ranges and averages of inflow percentages during 8 sampling events in
  # 2017 when the Fremont Weir was spilling
