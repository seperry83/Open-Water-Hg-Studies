...
loads_clean_c <- loads_clean %>%   
  complete(
    SamplingEvent, StationName, nesting(Analyte, AnalyteGroup, LoadUnits),
    fill = list(Load = 0)
  )

# Create a dataframe of these added zero load values to be removed from some of the
  # analyses later
zero_loads <- anti_join(loads_clean_c, loads_clean)

# Keep the zero load values in loads_clean df for now
loads_clean <- loads_clean_c

# Clean up
rm(ccsb_loads, ccsb_loads_sum, loads_inlet, loads_clean_c)
```
...
loads_clean_perc <- loads_clean %>% 
  ...
  mutate(percent_load = signif(Load/total_load, 3)) %>% 
  select(-c(AnalyteGroup:total_load)) %>% 
...
loads_clean_summ_no0 <- loads_clean17 %>% 
...
loads_clean_perc_summ_no0 <- loads_clean_perc %>% 
  filter(str_detect(SamplingEvent, "17$")) %>% 
  anti_join(zero_loads) %>% 
  summ_stat(percent_load, StationName, Analyte) %>% 
  mutate_at(vars(Mean:IQR), signif, digits = 3)
...
loads_clean_perc_frac <- loads_clean %>% 
  anti_join(zero_loads) %>% 
  filter(str_detect(AnalyteGroup, "Hg$")) %>% 
  ...
  mutate(
    perc_filt = signif(filtered/total, 3),
    perc_part = signif(particulate/total, 3)
  ) %>% 
  select(-c(filtered:total))
```

Calculate summary statistics of the % fractions for Hg and MeHg loads:
```{r calc summ stats perc fraction hg and mehg}
loads_clean_perc_frac_summ <- loads_clean_perc_frac %>% 
...
barplot_inlet_facet_fixed <- function(df) {
  p <- 
    ggplot(
      data = df,
      aes(
...
loads_barplots_fixed <- loads_clean17 %>% 
  anti_join(zero_loads) %>% 
  group_nest(Analyte) %>% 
  filter(str_detect(Analyte, "total$|TOC|TSS")) %>% 
  mutate(
    barplots_facet_fixed = map(data, .f = barplot_inlet_facet_fixed)
  )
...
```{r mehg inlet facet barplots fixed, fig.height = 5.5}
...
```{r hg inlet facet barplots fixed, fig.height = 5.5}
loads_barplots_fixed %>% 
...
```{r oc inlet facet barplots fixed, fig.height = 5.5}
loads_barplots_fixed %>% 
  filter(Analyte == "TOC") %>% 
  pull(barplots_facet_fixed) %>% 
  chuck(1) %>% 
...
```{r ss inlet facet barplots fixed, fig.height = 5.5}
loads_barplots_fixed %>% 
  filter(Analyte == "TSS") %>% 
  pull(barplots_facet_fixed) %>% 
  chuck(1) %>% 
...
loads_total_ff %>% filter(Analyte == "THg- total") %>% pull(perc_lower)
```
...
loads_clean_perc_fw_spill <- loads_clean_perc %>% 
  filter(
...
klrc_fw_spill <- loads_clean_perc_fw_spill$KLRC

# CCSB average during spill events
ccsb_fw_spill <- loads_clean_perc_fw_spill$CCSB
```
...
KLRC contributed an average of `r round(mean(klrc_fw_spill) * 100)`% of the uMeHg input load when the Fremont Weir was spilling. CCSB contributed an average of `r round(mean(ccsb_fw_spill) * 100)`%.
